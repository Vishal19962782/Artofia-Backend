...
const { append, redirect, render } = require("express/lib/response");
...
router.post(
  "/",
  check("mail").isEmail().withMessage("Enter a valid email address"),
  async (req, res) => {
      ...
      if (!user) {
        ...
        if (user.isBlocked) {
          res.status(403).render("index", {
            err: "User is blocked please contact admin",
            succ: "",
          });
        } else if (
          !user.isBlocked &&
          ...
          res
            .status(401)
            .render("index", { err: "Wrong username or password", succ: "" });
        }
      }
    } else {
      res
        .status(401)
        .render("index", { err: "Invalid Email Address", succ: "" });
    }
  }
);
debugger;
